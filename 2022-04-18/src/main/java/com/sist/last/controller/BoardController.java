...
@Controller
/*
 *    스프링에서 메모리 할당 : 패키지 단위로 메모리 할당 (선택정으로) → 스프링에서 관리
 *    @Controller
 *    @RestController
 *    @Repository
 *    @Service
...
public class BoardController {
    ...
    // 요청 처리 → @RequestMapping, @GetMapping, @PostMappping
    // HandlerMapping → 메소드 실행 HandlerAdapter
    @GetMapping("/board/list") // URI 주소(사용자의 브라우저에 잇는 주소)
    ...
    public String board_insert_ok(BoardEntity vo) {
        //dao.boardInsert(vo); // Insert SQL 문장이 첨부 → Update 사용이 가능
        // hibernate 기반 → DataSet
        dao.save(vo);
