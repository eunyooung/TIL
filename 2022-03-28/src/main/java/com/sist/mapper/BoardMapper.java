...
public interface BoardMapper {
    ...
    @Select("SELECT no,subject,name,TO_CHAR(regdate,'YYYY-MM-DD') as dbday,hit,num "
            ...
            + "FROM project_freeboard)) " + "WHERE num BETWEEN #{start} AND #{end}")
    ...
    // 서브쿼리, Sequence
    @SelectKey(keyProperty = "no", resultType = int.class, before = true, statement = "SELECT NVL(MAX(no)+1,1) as no FROM project_freeboard")
    @Insert("INSERT INTO project_freeboard VALUES(" + "#{no},#{name},#{subject},#{content},#{pwd}," + "SYSDATE,0)")
    public void boardInsert(BoardVO vo);

    // 상세보기 : 조건에 맞는 데이터 인출 @Update @Select → 분석 데이터 분석 (그래프)
    @Update("UPDATE project_freeboard SET " + "hit=hit+1 " + "WHERE no=#{no}")
    public void boardHitIncrement(int no);

    @Select("SELECT no,name,subject,content,hit," + "TO_CHAR(regdate,'YYYY-MM-DD') as dbday "
            + "FROM project_freeboard " + "WHERE no=#{no}")
