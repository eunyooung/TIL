...
public class MainClass4 {
    ...
    public static void main(String[] args) {
        ...
        try {
            // 파일을 읽는다 
            FileInputStream fis = new FileInputStream("c:\\javaDev\\data\\address.txt");
            ...
            while (true) {
                ...
                if (data == null)
                    break; // 읽을 데이터가 없는 경우 => null
                System.out.println(data);
                StringTokenizer st = new StringTokenizer(data, "|");
                // split() , replaceAll() => 정규식 
                Member m = new Member(Integer.parseInt(st.nextToken()), st.nextToken(), st.nextToken(), st.nextToken(),
                        st.nextToken());
                list.add(m);
            }
            fis.close();
            ...
            for (Member m : list) {
                System.out.println(
                        m.getNo() + " " + m.getName() + " " + m.getSex() + " " + m.getAddr() + " " + m.getTel());
            }
        } catch (Exception ex) {
