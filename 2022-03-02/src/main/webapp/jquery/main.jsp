  ...
  <script type="text/javascript">
    $(function() { // window.onload=function(){}, $(document).ready(function(){})
        $('#movieBtn').click(function() {
            // 영화 버튼을 클릭시에 처리하는 영역 
            $.ajax({ // 장점은 현재 파일에서 → 서버로 요청, 요청 결과값을 바로 받는다  
                // 파일을 계속 유지 → 70%이상 Ajax
                type : 'post', // 메소드 방식지정 (type)
                ...
                success : function(result) {
                    ...
                    /*
                         ...
                         → success:function(res){} → 시스템에 의해 자동 호출되는 함수
                            ...
                            ------- 오류가 없는 경우에 처리 
                                    res => 실행 결과값을 읽어 온다 
                         → error:function(res){} → error확인시 res=>에러코드
                            404  / 500 
                     */
                }
            })
...
<body>
  <div class="container">
    <div class="row">
      <input type=button value="영화" class="btn btn-sm btn-danger" id="movieBtn">
      <input type=button value="음악" class="btn btn-sm btn-success" id="musicBtn">
      <input type=button value="맛집" class="btn btn-sm btn-primary" id="foodBtn">
    </div>
    <div class="row print">
