...
public class DataBoardDAO {
    ...
    public DataBoardVO databoardUpdateData(int no) {
        ...
        try {
            getConnection();
            String sql = "SELECT name,subject,content " + "FROM databoard " + "WHERE no=?";
            ps = conn.prepareStatement(sql);
            ps.setInt(1, no);
            ResultSet rs = ps.executeQuery();
            rs.next();
    ...
    public boolean databoardDelete(int no, String pwd) {
        ...
        try {
            ...
            // 2-1 비밀번호 체크 
            String sql = "SELECT pwd,root,depth " + "FROM databoard " + "WHERE no=?";
            ps = conn.prepareStatement(sql);
            ps.setInt(1, no);
            ResultSet rs = ps.executeQuery();
            rs.next();
            ...
            if (db_pwd.equals(pwd)) {
                ...
                if (depth == 0) { // 답변이 없는 상태 → 삭제
                    ...
                    sql = "UPDATE databoard SET " + "depth=depth-1 " + "WHERE no=?";
                    ps = conn.prepareStatement(sql);
                    ps.setInt(1, root);
                    ps.executeUpdate();
                } else  { // 답변이 있는 상태 → 수정
