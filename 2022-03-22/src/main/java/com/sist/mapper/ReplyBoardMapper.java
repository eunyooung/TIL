...
public interface ReplyBoardMapper {
    ...
    */
    @Select("<script>" + "SELECT no,subject,name," + "TO_CHAR(regdate,'YYYY-MM-DD') as dbday,hit "
            + "FROM spring_replyboard " + "WHERE " + "<trim prefixOverrides='OR'>"
            + "<foreach collection='fsArr' item='fd'>" + "<trim prefix='OR'>" + "<choose>"
            + "<when test=\"fd=='N'.toString()\">" + "name LIKE '%'||#{ss}||'%'" + "</when>"
            + "<when test=\"fd=='S'.toString()\">" + "subject LIKE '%'||#{ss}||'%'" + "</when>"
            + "<when test=\"fd=='C'.toString()\">" + "content LIKE '%'||#{ss}||'%'" + "</when>" + "</choose>"
