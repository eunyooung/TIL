...
public interface RecipeMapper {
    ...
    @Select("SELECT no,title,poster,num " + "FROM (SELECT no,title,poster,rownum as num "
            + "FROM (SELECT /*+ INDEX_ASC(recipe recipe_no_pk)*/ no,title,poster " + "FROM recipe)) "
    ...
    @Select("SELECT chef,poster,num " + "FROM (SELECT chef,poster,rownum as num " + "FROM (SELECT chef,poster "
            + "FROM chef ORDER BY mem_cont1 DESC)) " + "WHERE num BETWEEN #{start} AND #{end}")
    ...
    // Chef가 만든 레시피 읽기
    @Select("SELECT no,poster,title,num " + "FROM (SELECT no,poster,title,rownum as num "
            + "FROM (SELECT /*+ INDEX_ASC(recipe recipe_no_pk)*/no,poster,title " + "FROM recipe WHERE chef=#{chef})) "
            + "WHERE num BETWEEN #{start} AND #{end}")
    ...
    @Select("SELECT no,title,poster,num " + "FROM (SELECT no,title,poster,rownum as num "
            + "FROM (SELECT /*+ INDEX_ASC(recipe recipe_no_pk)*/ no,title,poster " + "FROM recipe "
            + "WHERE title LIKE '%'||#{fd}||'%')) " + "WHERE num BETWEEN #{start} AND #{end}")
    public List<RecipeVO> recipeFindData(Map map);

    @Select("SELECT CEIL(COUNT(*)/12.0) " + "FROM recipe " + "WHERE title LIKE '%'||#{fd}||'%'")
    public int recipeFindTotalPage(String fd);

    // 추천 = 레시피 
    @Select("SELECT no,title,poster,num " + "FROM (SELECT no,title,poster,rownum as num "
            + "FROM (SELECT /*+ INDEX_ASC(recipe recipe_no_pk)*/no,title,poster " + "FROM recipe "
            + "WHERE REGEXP_LIKE(title,#{fd}))) " + "WHERE num BETWEEN #{start} AND #{end}")
    // 밥/죽/떡 → Like 3번 → REGEXP_LIKE(title, '밥|죽|떡')
    public List<RecipeVO> recipeRecommandData(Map map);

    @Select("SELECT CEIL(COUNT(*)/12.0) FROM recipe " + "WHERE REGEXP_LIKE(title,#{fd})")
    ...
    @Select({ "<script>"
            ...
            + "<trim prefix=\"OR\">"
            + "<choose>" + "<when test=\"fd=='T'.toString()\">"
            + "title LIKE '%'||#{ss}||'%'"
            + "</when>"
